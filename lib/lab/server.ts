...
export async function renderTool(id: string, input: Record<string, unknown>) {
  ...
  const { parts, ms } = await renderParts({ source: tool.source, values: job.scadValues, fonts, parts: job.parts });
  const soups = tool.parts.filter((p) => parts[p.id]).map((p) => ({ name: p.label, color: p.defaultColor, soup: parseStl(parts[p.id]) }));
  const box = bounds(soups.map((s) => s.soup));
  // Printable files, as the tool groups them (e.g. a base with pockets + separate pieces).
  const rendered = Object.keys(parts);
  const fileName = tool.fileName?.(values) ?? tool.id;
  const files = (tool.files?.(values, rendered) ?? rendered.map((id) => ({ id, label: id, parts: [id] }))).map((f) => ({
  ...
  return { tool, values, parts, soups, ms, size: box ? dimensions(box) : null, files, file3mf: multicolor ? build3mf(soups) : null };
}

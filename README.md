...
- **Enfeite para clipe**: envie um desenho em **SVG** e ele vira um enfeite colorido com uma **fenda para a ponta do clipe** na borda de baixo (padrão: 10 mm de largura, 1 mm de altura, 5 mm de profundidade; posição ajustável). A prévia mostra um **clipe virtual** encaixado, que dá para esconder.
- **Editor de desenho em tela cheia** (reaproveitável por outras ferramentas que partem de SVG): a peça em 3D ocupa a tela e o desenho visto de cima fica no canto (clique para trocar). Ferramentas à esquerda: **selecionar uma forma**, **selecionar tudo da mesma cor** (a mãozinha colorida), aproximar/afastar/centralizar, desfazer/refazer. Clique no 3D ou no 2D e a barra de baixo edita o que foi clicado: **filamento**, **mudar de cor** (juntar com outra ou criar uma nova), **altura em mm** e esconder. Painel **Camadas** com a visibilidade de cada cor; painéis **Cores** (cor do desenho → filamento), tamanho, encaixe e contorno; menu **Baixar**. Atalhos: V, C, T, ↑/↓ (altura), H (esconder), Esc, Ctrl+Z.
- O SVG é lido no próprio código (classes CSS, gradientes, `<use>`, transformações, arcos): cores quase iguais são juntadas, o fundo branco sai sozinho e o editor avisa o que ignorou (traços, textos, imagens). Cada cor vira uma peça, e uma forma pode ter cor e altura próprias; as cores ficam embutidas na base e nunca se sobrepõem, então o 3MF sai pronto para AMS.
